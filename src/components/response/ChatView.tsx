...
export function ChatView() {
  ...
  useEffect(() => {
    const unsubscribe = useResponseStore.subscribe((state, previousState) => {
      if (state.streamingContent === previousState.streamingContent) {
        return;
      }

      pendingStreamContentRef.current = state.streamingContent;
      ...
      streamFrameRef.current = requestAnimationFrame(() => {
        ...
        setStreamingDisplayContent(pendingStreamContentRef.current);
      });
    });
  ...
  useEffect(() => {
    ...
    autoFollowRef.current = isNearBottom(viewport);
  ...
  const completedMarkdown = useMemo(() => {
    ...
    return (
      <Markdown components={MARKDOWN_COMPONENTS}>{completedContent}</Markdown>
    );
  }, [completedContent]);
